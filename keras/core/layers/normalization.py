...
class BatchNormalization(Layer):
    ...
    def build(self, input_shape):
        ...
        self.input_spec = InputSpec(ndim=input_shape.ndims, axes={self.axis: param_shape[0]})
    ...
    def call(self, inputs, training=None):
        return normalization_ops.batch_norm(
            [inputs, self.gamma, self.beta, self.moving_mean, self.moving_variance],
            axis=self.axis,
...
class LayerNormalization(Layer):
    ...
    def call(self, inputs):
        return normalization_ops.layer_norm(
            [inputs, self.gamma, self.beta], axis=self.axis, epsilon=self.epsilon
        )
