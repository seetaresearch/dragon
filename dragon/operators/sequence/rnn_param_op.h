...
#ifndef DRAGON_OPERATORS_SEQUENCE_RNN_PARAM_OP_H_
#define DRAGON_OPERATORS_SEQUENCE_RNN_PARAM_OP_H_
 ...
 public:
  RNNParamSetOp(const OperatorDef& def, Workspace* ws)
    ...
    if (mode_str == "RNN_TANH") {
      num_params_ = 2, spliter_ = 1;
    } else if (mode_str == "RNN_RELU") {
      num_params_ = 2, spliter_ = 1;
    } else if (mode_str == "LSTM") {
      num_params_ = 8, spliter_ = 4;
    } else if (mode_str == "GRU") {
      num_params_ = 6, spliter_ = 3;
    } else {
...
#endif // DRAGON_OPERATORS_SEQUENCE_RNN_PARAM_OP_H_
