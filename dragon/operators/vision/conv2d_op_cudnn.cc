...
template <typename T>
void CuDNNConv2dOp<Context>::SetConvDesc() {
...
template <class Context>
template <typename T>
void CuDNNConv2dOp<Context>::DoRunWithType() {
...
template <class Context>
template <typename T>
void CuDNNConv2dGradientOp<Context>::SetConvDesc() {
  auto input_type = TypeMeta::Id<T>();
  if (input_type == TypeMeta::Id<float16>()) {
    compute_type_ = CUDNN_DATA_FLOAT;
  } else if (input_type == TypeMeta::Id<float>()) {
...
template <class Context>
template <typename T>
void CuDNNConv2dGradientOp<Context>::DoRunWithType() {
  auto &X = Input(0), &W = Input(1);
  auto *dX = Output(0), *dW = Output(1);
  ResetDesc<T>();
