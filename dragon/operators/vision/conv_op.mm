...
template <typename T>
void MPSConvOp<Context>::DoRunWithType() {
      ...
      [&](vector<MPSGraphTensor_t>& placeholders) {
        ...
        if (num_axes_ == 1 || num_axes_ == 2) {
          placeholders.emplace_back([graph_
              convolution2DWithSourceTensor:placeholders[0]
...
template <class Context>
template <typename T>
void MPSConvGradientOp<Context>::DoRunWithType() {
  ...
  auto placeholders = graph_cache_.GetPlaceholders(
      ...
      [&](vector<MPSGraphTensor_t>& placeholders) {
        ...
        if (num_axes_ == 1 || num_axes_ == 2) {
          placeholders.emplace_back([graph_
              convolution2DDataGradientWithIncomingGradientTensor:placeholders[2]
                                                    weightsTensor:placeholders[1]
                                                      outputShape:MPSGetShape(X_dims)
                                     forwardConvolutionDescriptor:conv2d_desc_
                                                             name:nil]);
          placeholders.emplace_back([graph_
              convolution2DWeightsGradientWithIncomingGradientTensor:placeholders[2]
...
DEPLOY_MPS_OPERATOR(Conv, MPSConv);
DEPLOY_MPS_OPERATOR(ConvGradient, MPSConvGradient);
