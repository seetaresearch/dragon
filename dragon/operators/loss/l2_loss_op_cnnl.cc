...
template <typename T>
void CNNLL2LossOp<Context>::DoRunWithType() {
...
template <class Context>
template <typename T>
void CNNLL2LossGradientOp<Context>::DoRunWithType() {
  ...
  if (reduction_ == "NONE") {
    ...
    const auto scale = 2.f / float(normalizer);
...
DEPLOY_CNNL_OPERATOR(L2Loss);
DEPLOY_CNNL_OPERATOR(L2LossGradient);
