...
template <class Context>
void BooleanMaskOp<Context>::RunOnDevice() {
...
void BooleanMaskGradientOp<Context>::DoRunWithType() {
  ...
  auto &X_ref = INPUT_SPEC(0), *X_index = Buffer("X_index");

  math::Set(
      dX->count(),
      convert::To<T>(0.f),
      dX->ReshapeLike(X_ref)->template mutable_data<T, Context>(),
      ctx());
...
template <class Context>
void BooleanMaskGradientOp<Context>::RunOnDevice() {
