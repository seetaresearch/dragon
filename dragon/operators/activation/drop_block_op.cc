...
void DropBlockOp<Context>::DoRunWithType() {
  ...
  if (phase() == "TEST") {
    ...
    auto axis = data_format() == "NCHW" ? 2 : 1;
    auto num_axes = X.ndim() - 2;
    int64_t feat_area = 1, mask_area = 1;
...
template <class Context>
void DropBlockOp<Context>::RunOnDevice() {
...
template <class Context>
void DropBlockGradientOp<Context>::RunOnDevice() {
  DispatchHelper<dtypes::Floating>::Call(this, Input(0));
}
