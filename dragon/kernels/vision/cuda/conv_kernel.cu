...
__global__ void _Im2Col2dNCHW(
  ...
  CUDA_1D_KERNEL_LOOP(i, nthreads) {
    ...
    for (int h_k = 0; h_k < kernel_h; h_k++) {
      for (int w_k = 0; w_k < kernel_w; w_k++) { // clang-format off
        ...
        *offset_col = (math::utils::IsAGeZeroAndALtB(h, H) &&
            ...
            ? math::utils::LDG(offset_im + (h_k * dilation_h * W + w_k * dilation_w))
            : T(0.f);
  ...
  CUDA_1D_KERNEL_LOOP(im_idx, nthreads) {
    ...
    for (int h_out = out_hstart; h_out < out_hend; ++h_out) {
      for (int w_out = out_wstart; w_out < out_wend; ++w_out) {
        int h_k = h - h_out * stride_h, w_k = w - w_out * stride_w;
        if (h_k % dilation_h == 0 && w_k % dilation_w == 0) {
          h_k /= dilation_h;
          w_k /= dilation_w;
          int col_idx = (c * kernel_h + h_k) * kernel_w + w_k;
          col_idx = (col_idx * out_h + h_out) * out_w + w_out;
          val += math::utils::LDGC<float>(col + col_idx);
        }
      }
    }
    im[im_idx] = val;
    ...
    for (int h_out = out_hstart; h_out < out_hend; ++h_out) {
      for (int w_out = out_wstart; w_out < out_wend; ++w_out) {
        ...
        if (h_k % dilation_h == 0 && w_k % dilation_w == 0) {
          h_k /= dilation_h;
          w_k /= dilation_w;
          int col_idx = h_out * out_w + w_out;
          col_idx = (col_idx * kernel_h + h_k) * kernel_w + w_k;
          val += math::utils::LDGC<float>(col + col_idx * C + c);
        }
      }
    }
    im[im_idx] = val;
...
#pragma unroll
      for (int d = D - 1; d >= 0; --d) {
        ...
        is_empty |= !math::utils::IsAGeZeroAndALtB(r, in_shape.data[d]);
      }
...
#pragma unroll
      ...
      if (!Transposed) {
        y[col_idx] = is_empty ? T(0.f) : math::utils::LDG(x + im_idx);
      } else if (!is_empty) {
...
DEFINE_KERNEL_LAUNCHER(Col2Im2d, true, bfloat16);
