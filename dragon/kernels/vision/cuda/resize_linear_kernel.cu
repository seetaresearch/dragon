...
__global__ void _ResizeLinear2dNCHW(
  ...
  CUDA_1D_KERNEL_LOOP(yi, nthreads) {
    ...
    const float tl = math::utils::LDGC<float>(x + (offset + ti) * W + li);
    const float tr = math::utils::LDGC<float>(x + (offset + ti) * W + ri);
    const float bl = math::utils::LDGC<float>(x + (offset + bi) * W + li);
    const float br = math::utils::LDGC<float>(x + (offset + bi) * W + ri);
    const float v = h - ti;
    const float u = w - li;
...
__global__ void _ResizeLinear2dNHWC(
  ...
  CUDA_1D_KERNEL_LOOP(yi, nthreads) {
    ...
    const float tl = math::utils::LDGC<float>(x + offset + (ti * W + li) * C);
    const float tr = math::utils::LDGC<float>(x + offset + (ti * W + ri) * C);
    const float bl = math::utils::LDGC<float>(x + offset + (bi * W + li) * C);
    const float br = math::utils::LDGC<float>(x + offset + (bi * W + ri) * C);
    const float v = h - ti;
    const float u = w - li;
    const float t = tl + (tr - tl) * u;
    const float b = bl + (br - bl) * u;
...
__global__ void _ResizeLinear2dGradNCHW(
  ...
  CUDA_1D_KERNEL_LOOP(yi, nthreads) {
    ...
    const float v = h - ti;
    const float u = w - li;
    const float dt = (1.f - v) * math::utils::LDGC<float>(dy + yi);
    const float db = v * math::utils::LDGC<float>(dy + yi);
