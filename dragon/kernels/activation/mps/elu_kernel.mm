...
kernel void Elu(
  ...
  const T val = x[index];
  y[index] = val > T(0) ? val : T(float_arg1) * (exp(val) - T(1));
}
...
kernel void EluGrad(
    ...
    const uint index [[thread_position_in_grid]]) {
  const T val = y[index];
  dx[index] = dy[index] * ((val > T(0)) ? T(1) : T(float_arg1) + val);
}
...
INSTANTIATE_KERNEL(Elu, half);
INSTANTIATE_KERNEL(Elu, float);
INSTANTIATE_GRAD_KERNEL(EluGrad, half);
INSTANTIATE_GRAD_KERNEL(EluGrad, float);
...
DEFINE_KERNEL_LAUNCHER(Elu, float16);
DEFINE_KERNEL_LAUNCHER(Elu, bfloat16);
DEFINE_KERNEL_LAUNCHER(Elu, float);
DEFINE_KERNEL_LAUNCHER(Elu, double);
DEFINE_GRAD_KERNEL_LAUNCHER(EluGrad, float16);
DEFINE_GRAD_KERNEL_LAUNCHER(EluGrad, bfloat16);
DEFINE_GRAD_KERNEL_LAUNCHER(EluGrad, float);
DEFINE_GRAD_KERNEL_LAUNCHER(EluGrad, double);
