...
class DataWorker(multiprocessing.Process):
    ...
    def __init__(self, **kwargs):
        ...
        """
        super(DataWorker, self).__init__(daemon=True)
    ...
    def get_outputs(self, inputs):
        ...
        # Decode.
        if inputs['encoded'] > 0:
            img = PIL.Image.open(io.BytesIO(inputs['data']))
        else:
            img = numpy.frombuffer(inputs['data'], numpy.uint8)
            img = img.reshape(inputs['shape'])
        label = numpy.array(inputs['label'], numpy.int64)
    ...
    def run(self):
        """Start the process to produce outputs."""
        numpy.random.seed(self._seed)

        while True:
            inputs = self._reader_queue.get()
            self._worker_queue.put(self.get_outputs(inputs))
