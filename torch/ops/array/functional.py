...
from dragon.vm.torch.ops.array import _functions
from dragon.vm.torch.tensor import Tensor


def argmax(input, dim=None, keepdim=False, out=None):
    """Return the indices of maximum elements.
...
def argmin(input, dim=None, keepdim=False, out=None):
    """Return the indices of minimum elements.
...
def expand(input, sizes):
    ...
    The length of ``sizes`` could either be less or
...
def nonzero(input, out=None):
    ...
    .. math:: \text{out} = \{i, \text{ if } \text{input}[i] \text{ is True }\}
