...
class ConversionContext {
 public:
    ...
    const ValueInfoMap& value_infos() const { return value_infos_; }
    const InitializerMap& initializer() const { return initializer_; }

    const int opset_version() const { return opset_version_; }
...
template <> const TensorProto* ONNXAttributes::get(const std::string& key) const;
...
class ONNXBackend {
 public:
    void BuildTensorFillOp(
        const TensorProto& onnx_tensor,
        OperatorDef* op_def);

    ONNXImporterReturns CommonONNXNodeImporter(
        ONNXNode*                           onnx_node,
        const ConversionContext&            ctx);

    ONNXImporterReturns ConvPoolNodeImporter(
        ONNXNode*                           onnx_node,
        const ConversionContext&            ctx);

    ONNXImporterReturns GemmNodeImporter(
        ONNXNode*                           onnx_node,
        const ConversionContext&            ctx);

    ONNXImporterReturns BatchNormNodeImporter(
        ONNXNode*                           onnx_node,
        const ConversionContext&            ctx);

    ONNXImporterReturns UpsampleNodeImporter(
        ONNXNode*                           onnx_node,
        const ConversionContext&            ctx);

    ONNXImporterReturns ReshapeNodeImporter(
        ONNXNode*                           onnx_node,
        const ConversionContext&            ctx);

    ONNXImporterReturns ATenNodeImporter(
        ONNXNode*                           onnx_node,
        const ConversionContext&            ctx);

    ONNXImporterReturns MaxRoiPoolNodeImporter(
        ONNXNode*                           onnx_node,
        const ConversionContext&            ctx);

    ONNXImporterReturns CastNodeImporter(
        ONNXNode*                           onnx_node,
        const ConversionContext&            ctx);

    ONNXImporterReturns LpNormNodeImporter(
        ONNXNode*                           onnx_node,
        const ConversionContext&            ctx);

    ONNXImporterReturns ArgReduceNodeImporter(
        ONNXNode*                           onnx_node,
        const ConversionContext&            ctx);
    ...
    using SpecialNodeConverter = ONNXImporterReturns(ONNXBackend::*)
        (ONNXNode*, const ConversionContext&);
