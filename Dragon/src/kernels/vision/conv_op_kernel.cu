        ...
        for (int kh = 0; kh < kernel_h; kh++) {
            for (int kw = 0; kw < kernel_w; kw++) {
                ...
                *y = (ih >= 0 && iw >= 0 && ih < H && iw < W) ?
                    __ldg(x + (kh * dilation_h * W + kw * dilation_w)) : T(0);
#else
                *y = (ih >= 0 && iw >= 0 && ih < H && iw < W) ?
                    x[kh * dilation_h * W + kw * dilation_w] : T(0);
#endif
                y += (out_h * out_w);
            }
        }
        ...
        for (int oh = oh_start; oh < oh_end; ++oh) {
            for (int ow = ow_start; ow < ow_end; ++ow) {
                ...
                if (kh % dilation_h == 0 &&
                    ...
                    kh /= dilation_h;
                    kw /= dilation_w;
                    yi = (((ic * kernel_h + kh) * kernel_w
                        + kw) * out_h + oh) * out_w + ow;
#if __CUDA_ARCH__ >= 350
