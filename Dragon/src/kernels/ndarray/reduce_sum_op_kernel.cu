...
template<> __global__ void _ColwiseReduceSum<half>(
    ...
    CUDA_2D_KERNEL_LOOP1(i, rows) {
        ...
        CUDA_2D_KERNEL_LOOP2(j, cols) {
            const int x_idx = i * cols + j;
            val += __half2float(__ldg(x + x_idx));
        }
...
void _ReduceSum(
    ...
    _GenericReduceSum<T>
        << < CUDA_2D_BLOCKS(outer_dim), CUDA_THREADS,
             0, ctx->cuda_stream() >> >
        (ndims, outer_dim, inner_dim, XSS, YDS, scale, x, y);
...
template <> void ReduceSum<float16, CUDAContext>(
    ...
    CUDAContext*            ctx) {
    _ReduceSum<half>(
        num_dims, dims, num_axes, axes, scale,
            reinterpret_cast<const half*>(x),
                reinterpret_cast<half*>(y), ctx);
}

/*! ReduceSumGrad <T = ?, Device = CUDA> */
...
#pragma unroll
        for (int d = ndim - 1; d >= 0; --d) {
            ...
            y_idx += (r % __ldg(y_dims + d)) * __ldg(y_strides + d);
#else
            FIXED_DIVISOR_DIV_MOD(x_dims[d], tmp, &tmp, &r);
            y_idx += (r % y_dims[d]) * y_strides[d];
#endif
        }
#if __CUDA_ARCH__ >= 350
        dx[x_idx] = __ldg(dy + y_idx) * scale;
#else
...
#pragma unroll
        ...
        dx[x_idx] = __float2half(__half2float(
            __ldg(dy + y_idx)) * scale);
#endif
    }
}
    ...
    template<> void ReduceSumGrad<T, CUDAContext>( \
        ...
        _ReduceSumGrad<T> \
            << < CUDA_BLOCKS(count), CUDA_THREADS, \
                 0, ctx->cuda_stream() >> > \
            (count, ndim, x_dims, y_dims, y_strides, scale, dy, dx); \
    }
...
template<> void ReduceSumGrad<float16, CUDAContext>(
    ...
    _ReduceSumGrad<half>
        << < CUDA_BLOCKS(count), CUDA_THREADS,
             0, ctx->cuda_stream() >> >
        (count, ndim, x_dims, y_dims, y_strides, scale, 
