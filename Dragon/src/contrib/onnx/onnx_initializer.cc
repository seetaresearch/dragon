...
void ConvertIntegralValue(
    ...
    if (converted) {
        for (const auto i : *t_src) values->add_ints(i);
    } else {
        const auto* int32_src = &onnx_tensor.int32_data();
        for (const auto i : *int32_src) values->add_ints(i);
    }
}

template <> void ConvertIntegralValue<google::protobuf::int64>(
    const TensorProto&              onnx_tensor,
    Argument*                       values) {
    auto* ints = values->mutable_ints();
    if (!TryConvertingTensorRawValues<
            google::protobuf::int64>(onnx_tensor, ints)) {
        ints->CopyFrom(onnx_tensor.int64_data());
    }
}

template <> void ConvertIntegralValue<google::protobuf::uint64>(
    const TensorProto&              onnx_tensor,
    Argument*                       values) {
    google::protobuf::RepeatedField<google::protobuf::uint64> tmp;
    const auto* src = &tmp;
    if (!TryConvertingTensorRawValues
            <google::protobuf::uint64>(onnx_tensor, &tmp)) {
        src = &onnx_tensor.uint64_data();
    } for (const auto i : *src) values->add_ints(i);
}
...
void ONNXBackend::BuildTensorFillOp(
    ...
    // Determine the shape
    auto* shape = op_def->add_arg(); shape->set_name("shape");
    for (const auto d : onnx_tensor.dims()) shape->add_ints(d);

    // Determine the data type and values
    auto* dtype = op_def->add_arg(); dtype->set_name("dtype");
    auto* values = op_def->add_arg(); values->set_name("values");
