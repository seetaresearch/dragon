...
void RAddOp<Context>::EltwiseRunImpl() {
...
template <class Context> template <typename T>
void RAddOp<Context>::BroadcastRunImpl(int type) {
    auto* a = X(0).template data<T, Context>();
    auto* b = X(1).template data<T, Context>();
    auto* y = Y(0)->template mutable_data<T, Context>();
...
template <class Context> template <typename T>
void RAddGradientOp<Context>::EltwiseRunImpl() {
...
template <class Context> template <typename T>
void RAddGradientOp<Context>::BroadcastRunImpl(int type) {
    DEFINE_INPUT_DESC;
    auto* dy = X(-1).template data<T, Context>();
    ...
    if (Y(1)->name() != "NULL") {
        auto* db = Y(0)->template mutable_data<T, Context>();
        math::Copy(Y(0)->count(), dy, db, ctx());
    }
}
