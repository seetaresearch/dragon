...
void SubOp<Context>::EltwiseRunImpl() {
...
template <class Context> template <typename T>
void SubOp<Context>::BroadcastRunImpl(int type) {
    auto* a = X(0).template data<T, Context>();
    auto* b = X(1).template data<T, Context>();
    auto* y = Y(0)->template mutable_data<T, Context>();
...
template <class Context> template <typename T>
void SubGradientOp<Context>::EltwiseRunImpl() {
...
template <class Context> template <typename T>
void SubGradientOp<Context>::BroadcastRunImpl(int type) {
    DEFINE_INPUT_DESC;
    auto* dy = X(-1).template data<T, Context>();
