...
void CuDNNConv2dOp<Context>::ResetDesc() {
    bool input_changed = (X(0).dims() != input_dims_);
    ...
    if (input_changed || filter_changed) {
        if (input_changed) {
            ...
            if (data_format() == "NCHW") {
                ...
                y_ofs_ = Y(0)->stride(0) / cudnn_group_;
...
template <class Context> template <typename T>
void CuDNNConv2dOp<Context>::RunImpl() {
...
template <class Context> template <typename T>
void CuDNNConv2dGradientOp<Context>::RunImpl() {
    ResetDesc<T>();
